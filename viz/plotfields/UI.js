...
class UI {
  ...
  constructor(config) {
    ...
    this.plotfields = config.plotfields; 
    this.setupUI()
  }
  ...
  setupUI() {
    ...
    if(this.turnOnPlanetName) {
      ...
      for(var i = 0; i < this.plotfields.lastFewPlanetN; i++) {
        $('#planettooltips')
  ...
  movePlanetFrame(config) {
    ...
    if(this.turnOnPlanetName) {
      ...
      var self = this;
      _.each(self.plotfields.lastFewPlanets, function(p, i) {


        $('#planettooltips_' + i).html(self.plotfields.planets[p].name);
        var thispos = new THREE.Vector3();
        thispos.setFromMatrixPosition(self.plotfields.planets[p].mesh.matrixWorld);
